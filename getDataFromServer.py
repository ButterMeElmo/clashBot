...
def getFileNames(startingFileName, extension, startingTimeStamp):
    ...
    endOfToday = datetime.datetime.utcnow().replace(
        tzinfo=pytz.utc).replace(hour=23, minute=59, second=59)

    while counter_aware_utc_dt <= endOfToday:
        results.append(getFileName(startingFileName,
                                   extension, counter_aware_utc_dt))
        counter_aware_utc_dt = counter_aware_utc_dt.replace(hour=12)
        counter_aware_utc_dt = counter_aware_utc_dt + \
