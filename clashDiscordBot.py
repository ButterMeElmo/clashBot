			...
			for i in range(len(accounts)):
				...
				async def waitForResult(account, message, ctx):
					...
					if result.reaction.emoji == config_strings.checkmark:
						resultStringTemp = clashAccessData.setMemberFreeGiftDayAndTime(account, dayOfWeek, hour, discordID)
						if resultStringTemp == config_strings.success:
							resultString = 'Set gift reminder for {}.'.format(account)
						else:
							resultString = 'Failed to set gift reminder for {}.'.format(account)
						await discordClient.send_message(ctx.message.channel,resultString)
						
				discordClient.loop.create_task(waitForResult(account, message, ctx))

	@commands.command(name='start', pass_context=True,  brief='Connect your discord to a clash account')
	async def start(self, ctx):
		messageToSay = "Hi!\nI\'m clashBot and I am going to help you connect your Clash of Clans account to discord.\nSometimes, I\'ll ask you questions. Usually, you'll type a response, but sometimes I will let you hit a check ({}) for yes or an x ({}) for no. Sound good?".format(config_strings.checkmark, config_strings.xmark)
	...
	async def linkAccountsBeta(self, ctx, discordID = None):
		...
		while moreAccountsToAdd == True:
			...
			if result == config_strings.successfully_linked_string:
				...
				await discordClient.add_reaction(message, config_strings.xmark)
				result = await discordClient.wait_for_reaction([config_strings.checkmark, config_strings.xmark], message=message, user = ctx.message.author)
				moreAccountsToAdd = (result.reaction.emoji == config_strings.checkmark)
		...
		allowedToDonate = clashAccessData.hasLinkedAccountWithTHLargerThan(discordID, config_options.minTHRequiredToBeADonator)
					
		if allowedToDonate:

			hasConfiguredIsTroopDonator = clashAccessData.hasConfiguredIsTroopDonator(discordID)
...
class ClanWar:
	...
	async def checkMyPastWarPerformance(self, ctx):
		...
		results = clashConvertDataToString.convert_war_attacks_to_string(results_dict)
		try:
			await discordClient.say(results)
		except:
	...
	async def checkMemberPastWarPerformance(self, ctx, number_of_wars = 5):
		while True:
			...
			clashAccountName = message.content.upper()
			try:
				resultDict = clashAccessData.getPastWarPerformanceForMemberName(clashAccountName, number_of_wars)
				resultString = clashConvertDataToString.convert_war_attacks_to_string(resultDict)
	...
	async def changeWarStatus(self, ctx):
		...
		accounts = clashAccessData.getLinkedAccountsList(discordID, currently_in_clan_required=True)		
		...
		if len(accounts) == 0:
			...
			for i in range(len(accounts)):
				...
				async def waitForResult(account, message, ctx):
					...
					result = await discordClient.wait_for_reaction([config_strings.checkmark, config_strings.xmark], message=message, user = ctx.message.author)
					if result.reaction.emoji == config_strings.checkmark:
						resultStringTemp = clashAccessData.addMemberToWar(account)
						if resultStringTemp == config_strings.success:
							resultString = '{} added to war'.format(account)
						else:
							resultString = 'Failed to add {} to war'.format(account)
					else:
						resultStringTemp = clashAccessData.removeMemberFromWar(account)
						if resultStringTemp == config_strings.success:
							resultString = '{} removed from war'.format(account)
						else:
							resultString = 'Failed to remove {} from war'.format(account)
					await discordClient.send_message(ctx.message.channel,resultString)
...
class ClanGames:
	...
	async def checkMembersCGScores(self, ctx):
		...
		while True:
			...
			message = await discordClient.wait_for_message(author=ctx.message.author)
			memberName = message.content.upper()
			results = clashAccessData.getClanGamesResultsForMemberName(memberName)
			await discordClient.say(results)
...
def updateTimesToCheckData():
	...
	currentRoundedDownToHourAnd5 = datetime.datetime(currentDateTime.year, currentDateTime.month, currentDateTime.day, currentDateTime.hour, 5).replace(tzinfo=pytz.utc)	
	print('currentRoundedDown {}'.format(currentRoundedDownToHourAnd5.timestamp()))
...
async def sendOutGiftReminders():
	...
	while True:
		...
		accountsWithGiftsOnIt = clashAccessData.getAccountsWhoGetGiftReminders(currentDateTime)
