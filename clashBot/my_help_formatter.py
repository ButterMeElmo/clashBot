	...
	def format(self, mobileFormat):
		...
		if self.is_bot():
			...
			for category, commands in itertools.groupby(data, key=category):
				...
				self._add_subcommands_to_page(max_width, commands, mobileFormat)
		else:
			self._paginator.add_line('Commands:')
			self._add_subcommands_to_page(max_width, self.filter_command_list(), mobileFormat)
...
def _default_help_command(ctx, *commands : str):
    ...
    if len(commands) == 0:
        ...
        all_commands_temp = sorted(bot.formatter.filter_command_list(), key=category)
        ...
        mypages = pages_modified2.Pages(bot, message=ctx.message, category_dict= category_dict)
