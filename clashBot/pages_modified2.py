...
class Pages:
    ...
    def __init__(self, bot, *, message, category_dict, per_page=9):
        ...
        for category_name, category_entries in category_dict.items():
            ...
            pages_for_category, left_over = divmod(len(category_entries), self.per_page)
...
#            ('\N{INFORMATION SOURCE}', self.show_help),
    ...
    def get_page(self, page):
        ...
        for category_name, category_entries in self.category_dict.items():
            ...
            if (len(category_entries) > self.per_page):
                ...
                number_of_pages_in_category, remainder = divmod(len(category_entries), self.per_page)
                if remainder:
                ...
                for i in range(0, number_of_pages_in_category):
                    if (current_page_checked + i) == page:
                        ...
                        commands_from_category_on_this_page = category_entries[start:(start + self.per_page)]     
                        command_names = []
    ...
    async def show_page(self, page, *, first=False):
        ...
        self.embed.set_footer(text='Page %s/%s (%s entries)' % (page, self.maximum_pages, self.total_entries))
