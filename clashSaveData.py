...
def addMemberToDB(cursor, tag, name, role, townHallLevel, lastSeenInWar):
    ...
    if role == None:
		...
		"""
        cursor.execute(query, (tag, name, tag, tag, townHallLevel,
                               lastSeenInWar, currentlyInClan, tag, tag, tag, tag, tag,))
		...
		"""
        cursor.execute(query, (tag, name, role, tag, tag,
                               tag, 1, tag, tag, tag, tag, tag))
    else:
...
#	print(warState)
    ...
    addWarToDB(cursor, friendlyTag, opponentTag, status, friendlyStars, opponentStars, friendlyDestructionPercentage,
               opponentDestructionPercentage, friendlyAttacks, opponentAttacks, warSize, prepStartTime, warStartTime, warEndTime)
...
#		print('setting to 0')
    for clanType in clanTypes:
        for member in war[clanType]['members']:
            ...
            if (clanType == 'clan'):
                addMemberToDB(cursor, memberTag, memberName,
                              None, memberTownHall, warEndTime)
                if not printedAlready:
            ...
            if 'attacks' not in member:
                ...
                attack1 = createEmptyWarAttack(
                    warID, memberTag, memberMapPosition, memberTownHall, 1, dataTime)
                attack2 = createEmptyWarAttack(
                    warID, memberTag, memberMapPosition, memberTownHall, 2, dataTime)
                ...
                for i in range(0, len(member['attacks'])):
                    ...
                    defenderPositionOnWarMap, defenderTownHall = findPositionAndTownHallForMemberTag(
                        war, defenderTag)
...
def markMembersNoLongerActive(cursor, profile):
    memberTagsPreviouslyActive = clashAccessData.getAllMembersTagSupposedlyInClan(
        cursor)
...
def processClanProfile(clanProfile, cursor):
    ...
    seasonID = getSeasonIDForUTCTimestamp(
        cursor, int(clanProfile['timestamp']/1000))
...
#	updateCurrentSeasonInDB(cursor, clanProfile)
    for member in clanProfile['memberList']:
        ...
        addDonationsToDB(cursor, tag, memberTag, troopsGiven,
                         troopsReceived, seasonID)
...
def addScannedDataToDB(cursor, data, scanned_data_index):
    ...
    query = """
		...
		VALUES
			(?, ?, ?, ?, ?, ?, ?, ?, ?, ?);
	"""
    cursor.execute(query, (member_tag, scanned_data_index, troops_donated_monthly, troops_received, spells_donated,
                           troops_donated_achievement, clan_games_points, attacks_won, defenses_won, town_hall_level))
...
def addMemberFromAchievements(entry, cursor, timestamp):
    ...
    cursor.execute(query, (tag, name, role, trophies, THLevel,
                           timestamp, tag, tag, tag, tag, tag, tag))
...
def getNextSeasonTimeStamp(timeBeingCalulatedFrom, extraMonth):
    ...
    aware_utc_dt = aware_utc_dt.replace(
        year=nextYear, month=nextMonth, day=1, hour=8, minute=0, second=0, tzinfo=pytz.utc)
    aware_utc_dt = aware_utc_dt - \
        datetime.timedelta(days=aware_utc_dt.weekday())
...
def validateSeasons(cursor, previousProcessedTime):
    ...
    previously_validated_season_id = clashAccessData.getSeasonIdForTimestamp(
        previousProcessedTime)
    current_season_id = clashAccessData.getSeasonIdForTimestamp(
        getDataFromServer.getUTCTimestamp())
    if full_run:
    ...
    for season_id in iterable:
        ...
        start_time_date_time = datetime.datetime.utcfromtimestamp(
            season_start_time)
        start_time_date_time_aware = start_time_date_time.replace(
            tzinfo=pytz.utc)

        start_of_interval = start_time_date_time_aware - \
            datetime.timedelta(hours=4)
        start_index = clashAccessData.get_min_index_greater_than_scanned_time(
            cursor, start_of_interval.timestamp())
        end_of_interval = start_of_interval + datetime.timedelta(hours=8)
        end_index = clashAccessData.get_min_index_greater_than_scanned_time(
            cursor, end_of_interval.timestamp())
...
#				if donates < 100 and received < 100 and attacks < 5 and defenses < 5:
            ...
            if (newSeasonVote / number_of_members_at_this_time) > .90:
                ...
                if possible_updated_start_timestamp > season_start_time and possible_updated_start_timestamp < season_end_time:
                    ...
                    print('this season seems to reset before: {}'.format(
                        possible_updated_start_timestamp))
...
def importSavedFreeGiftDays(cursor):
    ...
    for memberGiftDataEntry in data:
        ...
        cursor.execute(query, (free_item_day_of_week, free_item_hour_to_remind,
                               wants_war_reminder, wants_gift_reminder, member_tag))
...
def DEBUG_ONLY_getMemberNameFromTag(cursor, tag):
    ...
    if len(results) != 1:
        raise ValueError(
            'Theres not exactly one member name for member_tag: {}'.format(tag))
    return results[0][0]
...
#			debug = True
        ...
        for member_tag in current_data_set:
            ...
            troops_donated_monthly, troops_received_monthly, spells_donated_achievement, troops_donated_achievement, town_hall_level = current_data_set[
                member_tag]
            troops_donated_monthly_prev, troops_received_monthly_prev, spells_donated_achievement_prev, troops_donated_achievement_prev, town_hall_level = last_data_set[
                member_tag]

            # this is simple subtraction using previous achievements/scanned data
            member_donated_troops_and_spells_and_siege = troops_donated_monthly - \
                troops_donated_monthly_prev
            member_received_troops_and_spells_and_siege = troops_received_monthly - \
                troops_received_monthly_prev
            member_donated_spells = spells_donated_achievement - spells_donated_achievement_prev
            member_donated_troops_and_siege = troops_donated_achievement - \
                troops_donated_achievement_prev
        ...
        for member_tag in current_data_set:
            ...
            if troops_donated_this_period > 0 and debug:
                print('')
                print('TH {} donated total: {}'.format(
                    town_hall_level, troops_donated_this_period))
                print('TH {} donated spells: {}'.format(
                    town_hall_level, (spells_donated_achievement-spells_donated_achievement_prev)))
                print('TH {} donated troops: {}'.format(
                    town_hall_level, (troops_donated_achievement-troops_donated_achievement_prev)))
                print('')
        ...
        for member_tag in current_data_set:
            ...
            num_refills = math.ceil(
                troops_siege_spells_received / max_size_per_refill)
            ...
            if troops_siege_spells_received > 0 and debug:
                print('')
                print('TH {} received total: {} on {} refills'.format(
                    town_hall_level, troops_siege_spells_received, num_refills))
            ...
            if remainder <= (cc_spell_size + cc_siege_size) * num_refills and remainder > 0:
                ...
                for val in list_of_remainders:
                    ...
                    if val == 0:
                        ...
                        print('')
                        print('troops received: {}'.format(
                            troops_siege_spells_received))
                        ...
                        print('distributing received as: {}'.format(
                            list_of_remainders))
                        print('interpreted as {} spells'.format(
                            likely_spells_received))
                        print('interpreted as {} siege'.format(
                            likely_siege_received))
            ...
            if likely_siege_received > 0 and cc_siege_size == 0:
                raise ValueError(
                    'Examine these values to see why I think there is a siege machine when it cannot be')
        ...
        if number_of_spells_known_donated > 0 or number_of_spells_suspected_received > 0 or number_of_suspected_siege_machines_receieved > 0:
            if debug == True:
                ...
                print('num known spells donated: {}'.format(
                    number_of_spells_known_donated))
                print('num thought spells donated: {}'.format(
                    number_of_spells_suspected_received))
                print('num thought siege donated: {}'.format(
                    number_of_suspected_siege_machines_receieved))
                print('')
        ...
        for member_tag in current_data_set:
            ...
            troops_donated_during_this_period = troops_donated_monthly - \
                troops_donated_monthly_prev
            ...
            if member_tag in appears_to_have_received_siege_machines:
                number_of_siege_machines_suspected_excluding_this_member -= appears_to_have_received_siege_machines[
                    member_tag]
        ...
        for member_tag in possibly_donated_siege:
            ...
            if print_possible:
                ...
                print('there are {} possible sieges up for grabs, and {} had {} donates during this time'.format(
                    number_of_siege_machines_suspected_excluding_this_member, member_name, troops_donated_during_this_period))
                print('')
        ...
        if sum <= number_of_suspected_siege_machines_receieved:
            ...
            for member_tag in possibly_donated_siege:
                ...
                print('{} donated {} sieges'.format(
                    member_name, num_sieges_for_this_member))
            ...
            cursor.execute(query, (member_tag,))
            member_name = cursor.fetchall()[0][0]
            num_sieges_for_this_member = min(
                number_of_suspected_siege_machines_receieved, possibly_donated_siege[member_tag])
            print('{} donated {} sieges'.format(
                member_name, num_sieges_for_this_member))
...
def processSeasonData(cursor, previousProcessedTime):
    ...
    for season_id in iterable:
        ...
        if full_run:
            ...
            membersInClan = clashAccessData.getAllMembersTagSupposedlyInClan(
                cursor)
        ...
        for member_tag in membersInClan:
            ...
            try:
                min_index, max_index = clashAccessData.get_min_and_max_scanned_index_for_min_and_max_scanned_time(
                    cursor, season_start_time, season_end_time)
            ...
            query = '''INSERT OR REPLACE INTO SEASON_HISTORICAL_DATA (season_ID, member_tag, troops_donated, troops_received, spells_donated, attacks_won, defenses_won)	
				SELECT ?, ?, ?, ?, ?, ?, ?
				'''
            second_command_vars = (season_id, member_tag, total_troops_donated,
                                   total_troops_received, total_spells_donated, attacks_won, defenses_won)
            cursor.execute(query, second_command_vars)


def importClanGamesDetails(cursor):
    clanGames = json.load(
        open('manuallyInputtingDataConversion/1_clanGamesMain.json'))
    for clanGame in clanGames:
        ...
        query = """
			...
			VALUES
				...
				, ?, ?, ?, ?, ?);
		"""
        cursor.execute(query, (startTime, stopTime, startTime,
                               stopTime, topTierScore, personalCap, minTownHall))
...
def processClanGamesData(cursor, previousProcessedTime):
    ...
    for clanGame in clanGames:
        ...
        for memberTag in membersInClan:
            ...
            minAllowableTimeForClanGameData = getMinAllowableTimeForClanGameData(
                clanGames, clanGameID)
            try:
                min_index, max_index = clashAccessData.get_min_and_max_scanned_index_for_min_and_max_scanned_time(
                    cursor, minAllowableTimeForClanGameData, clanGameStartTime)
            except clashAccessData.NoDataDuringTimeSpanException:
        ...
        #	print(minAllowableTimeForClanGameData)
            ...
            if debug:
                ...
                print('minAllowableTimeForClanGameData: {}'.format(
                    minAllowableTimeForClanGameData))
#			print(timeBefore)
            if timeBefore == None:
                ...
                min_index, max_index = clashAccessData.get_min_and_max_scanned_index_for_min_and_max_scanned_time(
                    cursor, clanGameStartTime, clanGameEndTime)
                ...
                if timeBefore == None:
                    ...
                    print(DEBUG_ONLY_getMemberNameFromTag(
                        cursor, memberTag) + " seems to not be here during these games")
                    continue
...
#			print(timeBefore)
            ...
            maxAllowableTimeForClanGameData = getMaxAllowableTimeForClanGameData(
                clanGames, clanGameID)
            if debug:
                print('maxAllowableTimeForClanGameData: {}'.format(
                    maxAllowableTimeForClanGameData))
            ...
            if maxAllowableTimeForClanGameData == -2:
                if clanGameEndTime > processingTime:
                    ...
                    # most recent games
                    min_index = clashAccessData.get_min_index_greater_than_scanned_time(
                        cursor, clanGameEndTime)
                    query = '''
                ...
                min_index, max_index = clashAccessData.get_min_and_max_scanned_index_for_min_and_max_scanned_time(
                    cursor, clanGameEndTime, maxAllowableTimeForClanGameData)
                query = '''
					SELECT min(scanned_data_index) FROM 
						SCANNED_DATA 
					WHERE
...
#			print(memberTag)
            ...
            query = '''
				...
				'''
            if debug:
                print('inserting: cg # {}, {}, {}'.format(
                    clanGameID, memberTag, pointsScored))
                print('')
...
#			conn.set_trace_callback(print)
        ...
        warFileNames = getDataFromServer.getFileNames(
            'data/warDetailsLog', '.json', previousProcessedTime)
        ...
        clanInfoFileNames = getDataFromServer.getFileNames(
            'data/clanLog', '.json', previousProcessedTime)
        ...
        achievementsFileNames = getDataFromServer.getFileNames(
            'data/clanPlayerAchievements', '.json', previousProcessedTime)
            ...
            if os.path.exists(filename):
                ...
                for clanPlayerAcievementsEntry in clanPlayerAcievements:
                    if int(clanPlayerAcievementsEntry['timestamp'] / 1000) >= previousProcessedTime:
                        processClanPlayerAcievements(
                            clanPlayerAcievementsEntry, cursor)
        ...
        attemptToFindSiegeMachinesSinceLastProcessed(
            cursor, previousProcessedTime)
