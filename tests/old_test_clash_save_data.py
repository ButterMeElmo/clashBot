...
@pytest.mark.parametrize("tag, name, role, townHallLevel, lastSeenInWar", [
    ...
    pytest.param('taghesdgre', 'namehese', 'Elder', 3, 1064867, marks=pytest.mark.xfail(reason='the third one doesn\'t work yet')),
])
def test_addMemberToDB(db, tag, name, role, townHallLevel, lastSeenInWar):
	cursor = db.cursor()
	clashSaveData.addMemberToDB(cursor, tag, name, role, townHallLevel, lastSeenInWar)

	cursor.execute('SELECT * FROM MEMBERS')
	results = cursor.fetchall()
	desired_results = [(tag, name, role, None, townHallLevel, lastSeenInWar, 1, 0, None, None, None)]
	assert results == desired_results
...
def db_with_clan_and_members(db):
	...
	clashSaveData.addMemberToDB(cursor, test_member_tag_1, 'namehese', 'Elder', None, None)
	clashSaveData.addMemberToDB(cursor, test_member_tag_2, 'awegawhrh', 'Co-leader', None, None)
	yield db
...
def test_addDonationsToDB(db_with_clan_and_members, data):
	...
	for entry in data:
		...
		received = entry[3]
		season_id = entry[4]
		clashSaveData.addDonationsToDB(cursor, clan_tag, member_tag, donated, received, season_id)
	...
	for season_id in desired_results_pretransform:
		for member_tag in desired_results_pretransform[season_id]:
			...
			cursor.execute('SELECT * FROM TROOP_DONATIONS WHERE season_id = ? AND member_tag = ?', (season_id, member_tag))
			results = cursor.fetchall()
			desired_results = [
...
FAKE_WAR = (1, 'awghioe', 'iuawihgeawg', 'result_here', 5, 26, 16.754, 31.13, 1, 23, 20, 1234, 12345, 123456)
...
def test_addWarToDB(db, war_id, friendly_tag, enemy_tag, result, friendly_stars, enemy_stars, friendly_percentage, enemy_percentage, friendly_attacks_used, enemy_attacks_used, war_size, prep_day_start, war_day_start, war_day_end):
	...
	clashSaveData.addWarToDB(cursor, friendly_tag, enemy_tag, result, friendly_stars, enemy_stars, friendly_percentage, enemy_percentage, friendly_attacks_used, enemy_attacks_used, war_size, prep_day_start, war_day_start, war_day_end)
	cursor.execute('SELECT * FROM WARS')
	results = cursor.fetchall()
	desired_results = [(war_id, friendly_tag, enemy_tag, result, friendly_stars, enemy_stars, friendly_percentage, enemy_percentage, friendly_attacks_used, enemy_attacks_used, war_size, prep_day_start, war_day_start, war_day_end)]
	assert results == desired_results
...
def test_createEmptyWarAttack(warID, attackerTag, attackerPosition, attackerTownHall, attackNumber, dataTime):
	...
	result = clashSaveData.createEmptyWarAttack(warID, attackerTag, attackerPosition, attackerTownHall, attackNumber, dataTime)
...
@pytest.mark.parametrize("warID, attackerTag, attackerPosition, attackerTownHall, attackNumber, dataTime, defenderTag, defenderPosition, defenderTownHall, stars, destructionPercentage, attackOccurredAfter, orderNumber",
	[
		(1, 'aweawh', 12, 10, 1, 1760943, 'wagoehi', 1, 12, 3, 29.485, 128379, 54)
	])
def test_addWarAttackToDB(db_with_clan_and_members_and_war, warID, attackerTag, attackerPosition, attackerTownHall, attackNumber, dataTime, defenderTag, defenderPosition, defenderTownHall, stars, destructionPercentage, attackOccurredAfter, orderNumber):
	...
	warAttackDict = clashSaveData.createEmptyWarAttack(warID, attackerTag, attackerPosition, attackerTownHall, attackNumber, dataTime)
	...
	results = cursor.fetchall()
	desired_results = [(warID, attackerTag, defenderTag, attackerAttackNumber, attackerPosition, defenderPosition, attackerTownHall, defenderTownHall, stars, destructionPercentage, attackOccurredAfter, attackOccurredBefore, orderNumber)]
	assert results == desired_results
...
@pytest.mark.parametrize("patch_datetime_now, process_war_start, desired_results", [
		(
			...
			[
				(1,'#NG97789JF', '#YQPV0CPG', 'lost', 50, 60, 87.85, 100.0, 22, 27, 20, 1527208860, 1527291660, 1527378060),
				(2, '#NG97789JF', '#209CJ98QQ', 'in progress', 33, 40, 64.4, 75.4, 17, 23, 20, 1527553665, 1527636465, 1527722865),
				(3, '#NG97789JF', '#C0JQ082U', 'won', 59, 53, 98.4, 91.25, 32, 27, 20, 1527731402, 1527814202, 1527901570),
				(4, '#NG97789JF', '#G028GUQG', 'in progress', 0, 0, 0.0, 0.0, 0, 0, 20, 1527905298, 1527988098, 1528074498)
			]
		),
		pytest.param(
			process_war_end, 
			...
			[
				(1,'#NG97789JF', '#YQPV0CPG', 'lost', 50, 60, 87.85, 100.0, 22, 27, 20, 1527208860, 1527291660, 1527378060),
				(2, '#NG97789JF', '#209CJ98QQ', 'lost', 33, 40, 64.4, 75.4, 17, 23, 20, 1527553665, 1527636465, 1527722865),
				(3, '#NG97789JF', '#C0JQ082U', 'won', 59, 53, 98.4, 91.25, 32, 27, 20, 1527731402, 1527814202, 1527901570),
				(4, '#NG97789JF', '#G028GUQG', 'in progress', 0, 0, 0.0, 0.0, 0, 0, 20, 1527905298, 1527988098, 1528074498)
			]
...
def test_processWar(db, patch_datetime_now, process_war_start, desired_results):
	cursor = db.cursor()
	warFileNames = getDataFromServer.getFileNames('data/warDetailsLog', '.json', process_war_start.timestamp())
...
games_data = [(1, 1513670400, 1513843200, 10000, 500, None, 6), (2, 1513929600, 1514275200, 30000, 2000, None, 6), (3, 1515139200, 1515398400, 20000, 2500, None, 6), (4, 1515830400, 1515916800, 5000, 1000, None, 6), (5, 1516348800, 1516608000, 20000, 2500, None, 6), (6, 1517040000, 1517126400, 5000, 1000, None, 6), (7, 1517299200, 1517817600, 50000, 3000, None, 6), (8, 1517904000, 1518422400, 50000, 3000, None, 6), (9, 1518681600, 1519286400, 75000, 3000, None, 6), (10, 1519718400, 1520236800, 50000, 3000, None, 6), (11, 1520496000, 1520841600, 50000, 3000, None, 6), (12, 1521187200, 1521532800, 30000, 2000, None, 6), (13, 1521792000, 1522137600, 50000, 3000, None, 6), (14, 1522396800, 1522742400, 30000, 3000, None, 6), (15, 1523347200, 1523865600, 50000, 4000, None, 6), (16, 1524556800, 1525161600, 50000, 4000, None, 6), (17, 1525766400, 1526371200, 30000, 4000, None, 6), (18, 1526976000, 1527580800, 30000, 4000, None, 6), (19, 1528099200, 1528704000, 30000, 4000, None, 6), (20, 1529308800, 1529913600, 50000, 4000, None, 6)]
